...
from models.viz_utils import images_plot, line_plot
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if args.densityEstimation:  # logging for density estimation
            if batch_idx % args.log_every == 0:
                ...
                sys.stdout.write("\r")
                sys.stdout.write(
                    "| Epoch [%3d/%3d] Iter[%3d/%3d]\t\t%% bits/dim: %.3f Trace: %.3f  logp(z) %.3f"
                    % (
                        epoch,
                        args.epochs,
                        batch_idx + 1,
                        (len(trainset) // args.batch) + 1,
                        loss,
                        mean_trace,
                        mean_logpz,
                    )
                ...
                if args.log_verbose:
                    ...
                    grad_norm_inf = max(
                        p.grad.data.abs().max().item() for p in model.parameters() if p.grad is not None
                    )
            ...
            if batch_idx % 1 == 0:
                ...
                sys.stdout.write(
                    ...
                    % (
                        ...
                        (len(trainset) // args.batch) + 1,
                        loss.data.item(),
                        100.0 * correct.type(torch.FloatTensor) / float(total),
                    )
                )
                sys.stdout.flush()
        ...
        if args.densityEstimation:
            ...
            if batch_idx == 0:
                ...
                torchvision.utils.save_image(
                    samples.cpu(), os.path.join(im_dir, "samples_{}.jpg".format(epoch)), int(bs**0.5), normalize=True
                )
                torchvision.utils.save_image(
                    inputs.cpu(), os.path.join(im_dir, "data_{}.jpg".format(epoch)), int(bs**0.5), normalize=True
                )
                torchvision.utils.save_image(
                    x_re.cpu(), os.path.join(im_dir, "recons_{}.jpg".format(epoch)), int(bs**0.5), normalize=True
                )
