...
from models.utils_cifar import get_hms, interpolate, mean, std, test, train
from wikiart.wikiart_loader import WikiartDataset
...
parser.add_argument(
    "--powerIterSpectralNorm", default=5, type=int, help="number of power iterations used for spectral norm"
)
...
parser.add_argument("-nesterov", "--nesterov", dest="nesterov", action="store_true", help="nesterov momentum")
parser.add_argument("-norm", "--norm", dest="norm", action="store_true", help="compute norms of conv operators")
parser.add_argument(
    "-analysisTraceEst",
    "--analysisTraceEst",
    dest="analysisTraceEst",
    action="store_true",
...
parser.add_argument(
    "-noActnorm", "--noActnorm", dest="noActnorm", action="store_true", help="disable actnorm, default uses actnorm"
)
...
def anaylse_trace_estimation(model, testset, use_cuda, extension):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        # compute trace
        out_bij, p_z_g_y, trace, gt_trace = model(inputs[:, :, :8, :8], exact_trace=True)
...
def main():
    ...
    if args.dataset == "wikiart":
        dataset = WikiartDataset(batch_size=args.batch)

        trainset = dataset
        testset = dataset
        in_shape = (3, 300, 300)
        args.nClasses = 11  # Genres only for now
        ...
        testset = torchvision.datasets.MNIST(root="./data", train=False, download=False, transform=transform_test_mnist)
        args.nClasses = 10
        in_shape = (3, 32, 32)
    else:
    ...
    if args.deterministic:
        trainloader = torch.utils.data.DataLoader(
            ...
            shuffle=True if args.dataset != "wikiart" else False,
            num_workers=2,
            worker_init_fn=np.random.seed(1234),
        )
        testloader = torch.utils.data.DataLoader(
            testset, batch_size=args.batch, shuffle=False, num_workers=2, worker_init_fn=np.random.seed(1234)
        )
    else:
        trainloader = torch.utils.data.DataLoader(
            trainset, batch_size=args.batch, shuffle=True if args.dataset != "wikiart" else False, num_workers=2
        )
    ...
    if args.optimizer == "adam":
        ...
        optimizer = optim.SGD(
            model.parameters(), lr=args.lr, momentum=0.9, weight_decay=args.weight_decay, nesterov=args.nesterov
        )
